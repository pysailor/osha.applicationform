...
for pt in `find src/osha/applicationform/ -name "*.pt"` ; do bin/zptlint $pt; done
for xml in `find src/osha/applicationform/ -name "*.xml"` ; do bin/zptlint $xml; done
for zcml in `find src/osha/applicationform/ -name "*.zcml"` ; do bin/zptlint $zcml; done

echo '====== Running PyFlakes ======'
bin/zopepy setup.py flakes; handle_exit

echo '====== Running pep8 =========='
bin/pep8 --ignore=E501 --count src/osha/applicationform; handle_exit
